...
- 进入个人中心 https://console.cloud.tencent.com/developer，找到云函数
- 点击新建进入创建函数页面
- 依次选择 自定义创建,运行环境(CustomRuntime),本地上传zip包(上传zip包)
- 高级设置，描述(取自己喜欢的名字)
- 环境配置, 初始化超时时间(30s)，执行超时时间(30s)，这个没有固定，认为自己合适的时间就行。环境变量(scf_cookie)
- 触发器设置需要测试完才可以配置

### 1.3 第三部，测试函数执行
- 创建好函数后，进入函数管理，开始执行测试
